...
int KFInit(void) {
    ...
    // Step 5: Initialize XPF and locate kernel symbols (_msgbufp, _msgbufsize)
    extern int init_xpf(void);
    int xpf_ret = init_xpf();
    printf("[KFKernel] XPF init returned %d | msgbufp=0x%llx msgbufsize=0x%llx\n",
           xpf_ret, g_msgbufp_addr, g_msgbuf_size);
